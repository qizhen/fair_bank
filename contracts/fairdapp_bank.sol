...
contract FairBank is ERC721{
    ...
    uint256 public stageDuration;                   //阶段时间
    uint256 public standardProtectRatio;            //资金保护比率
    bool public activated = false;
    bool public modifyCountdown = false;
    
    uint256 public rId;                             //当前轮
    uint256 public sId;                             //当前阶段
    
    mapping (uint256 => FBankdatasets.Round) public round;  
    mapping (uint256 => mapping (uint256 => FBankdatasets.Stage)) public stage;
    
    mapping (address => bytes32) public register;                       //address => name
    mapping (bytes32 => address) public playerName;                     //name => address
    
    mapping (address => uint256[]) public playerGoodsList;              //用户投注列表
    mapping (address => uint256[]) public playerWithdrawList;           //用户提现列表
    ...
    FBankdatasets.Goods[] public goodsList;                             //所有投注记录
    
    FBankdatasets.Card[6] public cardList;                              //卡牌列表
    mapping (uint256 => address) public cardIndexToApproved;            //cID => address
        ...
        public
    {
        ...
        developerAddr.send(msg.value);                                      //直接发送给合约持有者？
    }
        ...
        public
    {
        ...
        uint256 _purchasePrice = cardList[_cId].amount.mul(110) / 100;                  //必须以1.1倍当前的价格购买
        ...
        if(cardIndexToApproved[_cId] != address(0)){                                    //原价的1.05付给上一任拥有者
            ...
            cardList[_cId].playerAddress.send(
                cardList[_cId].amount.mul(105) / 100                                    //原价的0.05付给开发者
                );
        ...
        payable
    {
        ...
        uint256 _sId = sId;
        uint256 _amount = msg.value;
        uint256 _targetExpectedAmount = getStageTargetAmount(_sId);             //隐藏函数的方法，如何计算
        uint256 _targetAmount = 
            stage[_rId][_sId].dividendAmount <= _targetExpectedAmount ?         //以大的为准
        ...
        private
    {
        ...
        uint256 _targetExpectedAmount = getStageTargetAmount(_sId);
        uint256 _targetAmount = 
            stage[_rId][_sId].dividendAmount <= _targetExpectedAmount ?                 //max(dividendAmount,_targetExpectedAmount)
            _targetExpectedAmount : stage[_rId][_sId].dividendAmount;
            _targetAmount = _targetAmount.mul(100) / 88;                                //实际筹款目标为100/88倍
        uint256 _stageTargetBalance =                                                   //筹款目标差额
            stage[_rId][_sId].amount > 0 ? 
            _targetAmount.sub(stage[_rId][_sId].amount) : _targetAmount;
        
        if(now > stage[_rId][_sId].start.add(stageDuration)                             //超过18个小时
            && _targetAmount > stage[_rId][_sId].amount                                 //没有达到筹款目标
        ){
            
            endRound(_rId, _sId);                                                       //结束本轮
        ...
        if(_stageTargetBalance > msg.value)                                             //购买金额小于筹款目标差额
            buyDataRecord(
                _rId, 
                _sId, 
                _targetAmount, 
            ...
            multiStake(                                                                 //购买金额大于筹款目标差额，直接开启下一阶段
                msg.value, 
                _stepSize, 
                _protectRatio, 
                _targetAmount, 
        ...
        returns(uint256, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        return
            (
                rId,
                sId,
                stage[rId][sId].start.add(stageDuration),                           //本阶段结束事件
                stage[rId][sId].amount,                                             //本阶段已筹集金额
                (                                                                   //本阶段目标筹集金额
                    stage[rId][sId].dividendAmount <= _targetExpectedAmount ? 
                    _targetExpectedAmount : stage[rId][sId].dividendAmount
                ).mul(100) / 88,
                round[rId].jackpotAmount.add(round[rId].amount.mul(88) / 100)       //本轮奖池金额
...
library FBankdatasets {
    ...
    struct Stage {
        uint256 start;
        uint256 amount;
        uint256 protectAmount;              //保证金
        uint256 dividendAmount;             //到期金额
    }
